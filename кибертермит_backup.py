...
<body>
    ...
    <script>
        ...
        fetch('/api/ip').then(r => r.json()).then(data => {
            document.getElementById('ipDisplay').textContent = `IP: ${data.ip}:5000`;
            document.getElementById('phoneStatus').textContent = 
                `Доступен по: ${data.ip}:5000`;
        });
        ...
        function showIP() {
            fetch('/api/ip').then(r => r.json()).then(data => {
                alert(`IP адрес сервера: ${data.ip}:5000\nОткройте этот адрес на телефоне!`);
            });
        }
...
if __name__ == '__main__':
    print("🤖 Запуск Кибер-Термита v1.0...")
    ...
    try:
        ...
        ip = s.getsockname()[0]
        s.close()
        print(f"   • http://{ip}:5000 (для телефона)")
    except:
